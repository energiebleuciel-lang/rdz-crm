...
async def generate_form_brief(selection: BriefSelectionForm, user: dict = Depends(get_current_user)):
    ...
    lines.append("")
    lines.append("⚠️ IMPORTANT : La clé API (api_key) est OBLIGATOIRE pour chaque envoi !")
    lines.append("")
    ...
    lines.append("{")
    lines.append(f'  "api_key": "{form.get("internal_api_key", "")}",  // ⚠️ OBLIGATOIRE - Clé API du formulaire')
    ...
    lines.append("```javascript")
    lines.append("// Configuration - NE PAS MODIFIER")
    lines.append(f"const API_KEY = '{form.get('internal_api_key', '')}';")
