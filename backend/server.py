...
async def generate_form_brief(selection: BriefSelectionForm, user: dict = Depends(get_current_user)):
    ...
    allowed_depts = form.get('allowed_departments', [])
    if allowed_depts and len(allowed_depts) > 0:
        lines.append(f"3️⃣ DÉPARTEMENT (RESTREINT à {len(allowed_depts)} département(s))")
        lines.append(f"   - Valeurs autorisées : {', '.join(sorted(allowed_depts))}")
        lines.append("   - Tous les autres départements REFUSÉS")
        lines.append("   - Bloquer l'étape suivante si département non autorisé")
    else:
        lines.append("3️⃣ DÉPARTEMENT (France métropolitaine uniquement)")
        lines.append("   - Valeurs autorisées : 01 à 95, 2A, 2B")
    ...
    # Générer le code de validation selon les départements autorisés
    if allowed_depts and len(allowed_depts) > 0:
        lines.append("// Validation département (LISTE RESTREINTE)")
        lines.append("function validateDepartement(dept) {")
        lines.append("  if (!dept) return false;")
        lines.append("  const d = dept.toString().toUpperCase().trim();")
        lines.append(f"  const allowed = [{', '.join([repr(d) for d in sorted(allowed_depts)])}];")
        lines.append("  return allowed.includes(d);")
        lines.append("}")
    else:
        lines.append("// Validation département France métropolitaine (TOUS)")
        lines.append("function validateDepartement(dept) {")
        lines.append("  if (!dept) return false;")
        lines.append("  const d = dept.toString().toUpperCase().trim();")
        lines.append("  // Corse")
        lines.append("  if (d === '2A' || d === '2B') return true;")
        lines.append("  // Métropole 01-95")
        lines.append("  const num = parseInt(d, 10);")
        lines.append("  return num >= 1 && num <= 95;")
        lines.append("}")
    lines.append("")
    lines.append("// Exemple d'utilisation")
    ...
    if allowed_depts and len(allowed_depts) > 0:
        lines.append(f"    alert('Département non autorisé. Départements acceptés : {', '.join(sorted(allowed_depts)[:10])}...');")
    else:
        lines.append("    alert('Département invalide. France métropolitaine uniquement (01-95, 2A, 2B)');")
    lines.append("    return false;")
    lines.append("  }")
